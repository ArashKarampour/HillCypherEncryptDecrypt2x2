...
public class HillCipher {
    ...
    private static int[][] getKeyMatrix(String key,final JFrame frame) {  
        ...
        if (sq != (long) sq) {  
            ...
            JOptionPane.showMessageDialog(frame, "Cannot Form a square matrix!",
    ...
    private static void isValidMatrix(int[][] keyMatrix,final JFrame frame) {  
        int det = keyMatrix[0][0] * keyMatrix[1][1] - keyMatrix[0][1] * keyMatrix[1][0];  
        // If det=0, throw exception and terminate  
        if(det == 0) { 
        	JOptionPane.showMessageDialog(frame, "Det equals to zero, invalid key matrix!",
                    "ERROR", JOptionPane.ERROR_MESSAGE);
    ...
    // This method checks if the reverse key matrix is valid (matrix mod26 = (1,0,0,1)  
        private static void isValidReverseMatrix(int[][] keyMatrix, int[][] reverseMatrix,final JFrame frame) {  
        int[][] product = new int[2][2];  
        // Find the product matrix of key matrix times reverse key matrix  
        product[0][0] = (keyMatrix[0][0]*reverseMatrix[0][0] + keyMatrix[1][0] * reverseMatrix[0][1]) % 26;  
        product[0][1] = (keyMatrix[0][1]*reverseMatrix[0][0] + keyMatrix[1][1] * reverseMatrix[0][1]) % 26;  
        product[1][0] = (keyMatrix[0][0]*reverseMatrix[1][0] + keyMatrix[1][0] * reverseMatrix[1][1]) % 26;  
        product[1][1] = (keyMatrix[0][1]*reverseMatrix[1][0] + keyMatrix[1][1] * reverseMatrix[1][1]) % 26;  
        ...
        if(product[0][0] != 1 || product[0][1] != 0 || product[1][0] != 0 || product[1][1] != 1) {
        	JOptionPane.showMessageDialog(frame, "Invalid reverse matrix found!",
                    "ERROR", JOptionPane.ERROR_MESSAGE);
    ...
    private static String echoResult(String label, int adder, ArrayList<Integer> phrase) {  
        int i;
        String out="";
        out+=(label)+" ";  
        // Loop for each pair  
        for(i=0; i < phrase.size(); i += 2) {  
            out+=(Character.toString((char) (phrase.get(i) + (64 + adder))));  
            out+=(Character.toString((char) (phrase.get(i+1) + (64 + adder))));  
            if(i+2 <phrase.size()) {  
                out+=("-");  
            }  
        }  
    ...
    public static String encrypt(String phrase, boolean alphaZero,String keyMat,final JFrame frame)  
    {  
        int i;  
        ...
        // Get the 2x2 key matrix  
        keyMatrix = getKeyMatrix(keyMat,frame);  
        // Check if the matrix is valid (det != 0)  
        isValidMatrix(keyMatrix,frame);  
        //System.out.println(keyMatrix[0][0]+" "+keyMatrix[0][1]+" "+keyMatrix[1][0]+" "+keyMatrix[1][1]);
        ...
        for(i=0; i < phraseToNum.size(); i += 2) {  
            int x = (keyMatrix[0][0] * phraseToNum.get(i) + keyMatrix[1][0] * phraseToNum.get(i+1)) % 26;  
            int y = (keyMatrix[0][1] * phraseToNum.get(i) + keyMatrix[1][1] * phraseToNum.get(i+1)) % 26;  
            phraseEncoded.add(alphaZero ? x : (x == 0 ? 26 : x ));  
            phraseEncoded.add(alphaZero ? y : (y == 0 ? 26 : y ));  
        }  
        // Print the result  
        return echoResult("Encoded phrase: ", adder, phraseEncoded);  
    }  
    // This method makes the actual decryption  
    public static String decrypt(String phrase, boolean alphaZero,String keyMat,final JFrame frame)  
    {  
        int i; 
        ...
        //System.out.println(revKeyMatrix[0][0]+" "+revKeyMatrix[0][1]+" "+revKeyMatrix[1][0]+" "+revKeyMatrix[1][1]);
        // Check if the reverse key matrix is valid (product = 1,0,0,1)  
        isValidReverseMatrix(keyMatrix, revKeyMatrix,frame);  
        ...
        //System.out.println(phraseDecoded);
        // Print the result  
        return echoResult("Decoded phrase: ", adder, phraseDecoded);  
    }
